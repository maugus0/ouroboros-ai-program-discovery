...
from app.llm.schemas import ExtractedProgramData, FieldClassification, ParsedRequirements
from app.models.ranking import LLMExtractionResult
...
class LLMService:
    ...
    async def extract_program(self, page_text: str, source_url: str) -> LLMExtractionResult:
        """Extract structured program data from page text using LLM.
        ...
        system_prompt = get_program_extraction_prompt(context=runtime_context, fmt="text")
        ...
        try:
            if settings.OPENAI_API_KEY:
                ...
                program = ExtractedProgramData(**result["content"])
                latency = int((time.perf_counter() - start) * 1000)
                return LLMExtractionResult(
                    extracted_data=program.model_dump(),
    ...
    async def parse_requirements(self, requirements_text: str) -> list[dict[str, Any]]:
        """Parse natural language requirements into structured entries."""
        system_prompt = get_requirement_parsing_prompt(fmt="text")
        user_content = f"REQUIREMENT TEXT:\n{requirements_text}"

        try:
            if settings.OPENAI_API_KEY:
                result = await call_openai(system_prompt, user_content)
                parsed = ParsedRequirements(**result["content"])
                return [r.model_dump() for r in parsed.requirements]
        except Exception as exc:  # pylint: disable=broad-exception-caught
            logger.warning("openai_requirement_parsing_failed", error=str(exc))
        ...
        try:
            ...
            logger.error("anthropic_requirement_parsing_failed", error=str(exc))

        return []

    async def classify_field(self, program_name: str, description: str | None = None) -> dict[str, Any]:
        """Classify a program into a standard field and category."""
        system_prompt = get_field_classification_prompt(fmt="text")
        user_content = f"PROGRAM NAME: {program_name}"
        if description:
            user_content += f"\n\nDESCRIPTION:\n{description}"
        ...
        return {"field": program_name, "field_category": None, "confidence": 0.0}
