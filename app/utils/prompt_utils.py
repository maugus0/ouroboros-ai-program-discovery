...
def load_prompt_template(filename: str) -> dict[str, Any]:
    ...
    Args:
        filename: Name of the JSON file in prompts/
...
def merge_runtime_context(
    ...
    Args:
        ...
        context: Runtime context to merge

    Returns:
        Merged prompt dictionary
    """
...
def build_prompt_json(
    ...
    Args:
        template_file: Filename of the JSON template
        context: Runtime context to inject

    Returns:
        JSON-formatted string ready for LLM consumption
    """
...
def get_prompt_version(template_file: str) -> str:
    """Extract version from prompt template filename.

    Args:
        template_file: Filename like 'program_extraction_v1.json'

    Returns:
        Version string like 'v1' or 'unknown'
    """
...
def _clean_context(context: dict[str, Any]) -> dict[str, Any]:
    ...
    for drop_key in ("agent_capabilities", "internal_flags", "api_keys"):
...
def _label(key: str) -> str:
    """Convert snake_case key to UPPER CASE LABEL."""
    return key.upper().replace("_", " ")


def _format_section(value: Any, indent: int = 0) -> str:
    """Recursively format a section value as readable text."""
    prefix = "  " * indent
