"""LLM-assisted program data extraction from HTML content."""

from typing import Any

from app.core.logging import get_logger
from app.crawlers.parsers.html_parser import extract_basic_metadata, extract_page_text
from app.llm.schemas import ExtractedProgramData
...
async def extract_program_metadata(html: str, source_url: str) -> dict[str, Any]:
    """Extract program data with fallback to BeautifulSoup if LLM fails."""
    page_text = extract_page_text(html)
    truncated = truncate_text(page_text)

    try:
        llm_service = LLMService()
        result = await llm_service.extract_program(truncated, source_url)
