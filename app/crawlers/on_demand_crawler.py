...
from app.crawlers.parsers.llm_parser import extract_program_metadata
...
async def crawl_program_page(url: str) -> dict[str, Any] | None:
    """Fetch a program page and extract metadata using LLM-assisted parsing."""
    html = await fetch_page(url)
    if html is None:
        return None

    metadata = await extract_program_metadata(html, url)
    metadata["source_url"] = url
    return metadata


async def discover_program_links(programs_page_url: str) -> list[str]:
    """Crawl a university's programs listing page and discover individual program links."""
    html = await fetch_page(programs_page_url)
    if html is None:
        return []

    links = extract_links(html, programs_page_url)
    logger.info("program_links_discovered", url=programs_page_url, count=len(links))
    return links
