...
def test_load_program_extraction_template():
    template = load_prompt_template("program_extraction_v1.json")
    assert "prompt_template" in template
    assert "base" in template["prompt_template"]


def test_load_requirement_parsing_template():
    template = load_prompt_template("requirement_parsing_v1.json")
    assert "prompt_template" in template
...
def test_build_prompt_json_without_context():
    result = build_prompt_json("program_extraction_v1.json")
...
def test_build_prompt_json_with_context():
    ...
    result = build_prompt_json("program_extraction_v1.json", context)
    assert "source_url" in result
    assert "runtime_context" in result


def test_build_prompt_text():
    result = build_prompt_text("program_extraction_v1.json")
    assert isinstance(result, str)
